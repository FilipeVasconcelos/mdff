...
if $mdff; then
	...
	cd mdff
		...
		echo "NVT velocities rescaled PIM"
		cp ../config/POSFF.SiO2 POSFF
		$EXEMDFF control_md_full_run_pim_nvt.F > stdout_md_full_run_pim_nvt
		echo "full run finish"
		mv OSZIFF OSZIFF.full_pim_nvt
		$EXEMDFF control_md_splited_run1_pim_nvt.F > stdout_md_splited_run1_pim_nvt
		echo "split 1 finish"
		mv OSZIFF OSZIFF.run1_pim_nvt
		$EXEMDFF control_md_splited_run2_pim_nvt.F > stdout_md_splited_run2_pim_nvt
		echo "split 2 finish"
